...
export default async function UserPostsPage({ params }: Props) {
  ...
  const targetProfile = await getProfileByUsername(supabase, params.username);
  if (!targetProfile || targetProfile.status !== 'active') {
    notFound();
  }

  // 2. Auth check and check if logged-in user is the owner
  const loggedInProfile = await getCurrentProfile(supabase);
  const isOwner = loggedInProfile !== null && loggedInProfile.id === targetProfile.id;

  let posts: CreatorPostWithDetails[] = [];
  let approved = 0;
  let pending = 0;
  let rejected = 0;

  if (isOwner) {
    // Only creators can manage their posts
    if (loggedInProfile.role !== 'creator') {
      redirect(`/u/${loggedInProfile.username}`);
    }
    const { data: ownPosts } = await listCreatorOwnPosts(supabase);
    posts = ownPosts;
    approved = posts.filter((p: CreatorPostWithDetails) => p.moderation_status === 'approved').length;
    pending = posts.filter((p: CreatorPostWithDetails) => p.moderation_status === 'pending').length;
    rejected = posts.filter((p: CreatorPostWithDetails) => p.moderation_status === 'rejected').length;
  } else {
    // Visitor view: Fetch target creator profile
  ...
  return (
    <main className="max-w-5xl mx-auto px-4 sm:px-6 py-8 space-y-8">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <h1 className="text-2xl font-black text-gray-900 mt-2 tracking-tight">
            {isOwner ? 'Work Posts' : `@${targetProfile.username}'s Posts`}
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            {isOwner
              ? 'Share your work — posts go live instantly.'
              : `Browse creative work and portfolio updates from @${targetProfile.username}.`}
          </p>
        </div>
        {isOwner && (
          <Link
      ...
      {/* Stats row & moderation info (Owner Only) */}
      {isOwner && (
        <>
      ...
      {posts.length === 0 ? (
          ...
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1 max-w-xs">
              {isOwner
                ...
                : 'This user has not published any work posts yet.'}
            </p>
          </div>
          {isOwner && (
            <Link
        ...
        <CreatorPostGrid posts={posts} showCreator={false} isOwnDashboard={isOwner} />
      )}
    </main>
  );
}
