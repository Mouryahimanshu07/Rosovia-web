...
alter table public.listings
  drop constraint if exists listings_reserved_stock_check,
  add constraint listings_reserved_stock_check check (reserved_stock >= 0);

alter table public.listings
  drop constraint if exists listings_sold_stock_check,
  add constraint listings_sold_stock_check check (sold_stock >= 0);

alter table public.listings
  drop constraint if exists listings_stock_reservation_check,
